...
func run(obj : Object, delta : float):
	...
	if Input.is_action_pressed("btn_left"):
		...
		if abs(obj.vel.x) < 1:
			...
			is_moving = true
	if not obj.is_jumping and not obj.double_jump and jump_margin_timer > 0:
		jump_margin_timer -= delta
		if Input.is_action_just_pressed("btn_jump"):
			obj.fsm.state_nxt = obj.fsm.STATES.jump
	if obj.is_jumping and not obj.double_jump and \
